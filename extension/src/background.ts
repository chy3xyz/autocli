...
async function handleCommand(cmd: Command): Promise<Result> {
  // ── Protocol adapter: normalize CLI Zig format to extension internal format ──
  // CLI sends: {"action":"evaluate","params":{"expression":"..."},"tab_id":"123"}
  // Extension expects: {"action":"exec","code":"...","tabId":123}
  const normalized = normalizeCliCommand(cmd);
  const workspace = getWorkspaceKey(normalized.workspace);
  // Reset idle timer on every command (window stays alive while active)
  resetWindowIdleTimer(workspace);
  try {
    switch (normalized.action) {
      case 'exec':
        return await handleExec(normalized, workspace);
      case 'navigate':
        return await handleNavigate(normalized, workspace);
      case 'tabs':
        return await handleTabs(normalized, workspace);
      case 'cookies':
        return await handleCookies(normalized);
      case 'screenshot':
        return await handleScreenshot(normalized, workspace);
      case 'close-window':
        return await handleCloseWindow(normalized, workspace);
      case 'cdp':
        return await handleCdp(normalized, workspace);
      case 'sessions':
        return await handleSessions(normalized);
      case 'set-file-input':
        return await handleSetFileInput(normalized, workspace);
      case 'read-article':
        return await handleReadArticle(normalized, workspace);
      default:
        return { id: cmd.id, ok: false, error: `Unknown action: ${normalized.action}` };
    }
  } catch (err) {
    return {
      id: cmd.id,
...
// Translates Zig CLI page.zig commands into extension-native actions.

/** Raw shape of a command arriving from the Zig daemon (may include CLI-specific fields). */
interface CliRawCommand {
  id: string;
  action: string;
  tab_id?: string;
  tabId?: number;
...
function normalizeCliCommand(raw: CliRawCommand): Command {
  const p = raw.params ?? {};
  const base: Partial<Command> = {
    id: raw.id,
    workspace: raw.workspace,
    tabId: raw.tabId ?? (raw.tab_id ? Number(raw.tab_id) : undefined),
  };

  switch (raw.action) {
    // ── Direct pass-through (extension-native actions) ──
    case 'exec':
    case 'navigate':
    case 'tabs':
    case 'screenshot':
    ...
    case 'set-file-input':
    case 'read-article':
      return { ...base, ...raw } as Command;

    // ── CLI page.zig actions → extension actions ──
    case 'goto':
      return { ...base, action: 'navigate', url: (p.url as string) ?? raw.url };

    case 'evaluate':
      return { ...base, action: 'exec', code: (p.expression as string) ?? (p.code as string) ?? '' };

    case 'url':
      return { ...base, action: 'exec', code: 'window.location.href' };

    case 'title':
      return { ...base, action: 'exec', code: 'document.title' };

    case 'content':
      return { ...base, action: 'exec', code: 'document.documentElement.outerHTML' };

    case 'click':
      return { ...base, action: 'exec', code: `document.querySelector(${JSON.stringify(p.selector)})?.click()` };

    case 'type': {
      const sel = JSON.stringify(p.selector);
      const txt = JSON.stringify(p.text ?? '');
      return { ...base, action: 'exec', code: `(()=>{const e=document.querySelector(${sel});if(e){e.focus();e.value=${txt};e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}))}})()` };
    }

    case 'wait_for_selector': {
      const sel = JSON.stringify(p.selector);
      return { ...base, action: 'exec', code: `new Promise((r,j)=>{const t=setTimeout(()=>j('timeout'),30000);const c=()=>{if(document.querySelector(${sel})){clearTimeout(t);r(true)}else{requestAnimationFrame(c)}};c()})` };
    }

    case 'wait_for_navigation':
      return { ...base, action: 'exec', code: 'new Promise(r=>{if(document.readyState==="complete"){r(true)}else{window.addEventListener("load",()=>r(true),{once:true})}})' };

    case 'wait_for_timeout':
      return { ...base, action: 'exec', code: `new Promise(r=>setTimeout(r,${p.ms ?? 1000}))` };

    case 'cookies':
      return { ...base, action: 'cookies', domain: p.domain as string };

    case 'set_cookies':
      // set_cookies is a CDP operation — forward as cdp action
      return { ...base, action: 'cdp', cdpMethod: 'Network.setCookies', cdpParams: { cookies: p.cookies } };

    case 'snapshot':
      return { ...base, action: 'cdp', cdpMethod: 'Accessibility.getFullAXTree' };

    case 'auto_scroll':
      return { ...base, action: 'exec', code: '(()=>{let h=document.documentElement.scrollHeight,i=0;const s=()=>{if(i<10){window.scrollBy(0,300);i++;requestAnimationFrame(s)}};s()})()' };

    case 'switch_tab':
      return { ...base, action: 'tabs', op: 'select', index: p.index as number };

    case 'close':
      return { ...base, action: 'tabs', op: 'close' };

    case 'intercept_requests':
      return { ...base, action: 'cdp', cdpMethod: 'Network.enable' };

    case 'get_intercepted_requests':
    case 'get_network_requests':
      return { ...base, action: 'cdp', cdpMethod: 'Network.getResponseBody' };

    // ── Unknown — pass through as-is ──
    default:
      return { ...base, action: raw.action } as Command;
  }
}
